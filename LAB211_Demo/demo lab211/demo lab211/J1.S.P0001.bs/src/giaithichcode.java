...
*/
        ...
        while (true) {
            ...
            try {
                ...
                /*Đề bài yêu cầu bạn dùng bubble sort sắp sếp 1 mảng random, vì số các phần tử mảng phải > 0
                  Nếu điều kiện kiểm tra đúng(result <= 0) thì một NumberFormatException sẽ được ném ra. 
    ...
    //Phương thức dùng để sinh ngẫu nhiên các phần tử trong mảng với giá trị từ 0 đến 9.
    private static void random(int n, int arr[]){
         Random rand = new Random();
        for (int i = 0; i < n; i++) {   
        arr[i] = rand.nextInt(10);     //bạn có thể thay đổi giá trị từ 0 --> bất kì                 
        }
    }
    ...
    */
    ...
    int arr[] = new int[n]; //Khởi tạo một mảng arr với kích thước đã nhập từ bàn phím.
        random(n, arr); //Gọi hàm random để sinh ngẫu nhiên các phần tử trong mảng arr.
